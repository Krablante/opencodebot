...
Use `/context`, `/context N`, or `/set_context N` to export the latest completed main-session prompt/final-answer pairs from the current topic. The personal default is three pairs and the supported range is 1–10. The bot excludes tool/intermediate assistant messages, subagent sessions, synthetic prompts, and the active unfinished turn; it preserves user attachment descriptors. Output is one or more collapsed Rich Messages containing an escaped code block, split without truncation below the safe Rich Message limit and capped at 240,000 characters. Expanding the block exposes Telegram's native code-copy affordance. Context text is never stored in bot state and a Rich Message failure produces only a short error, never a large plain-text fallback.
