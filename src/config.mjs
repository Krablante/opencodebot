...
const defaultMirror = { deletePinServiceMessages: true, pinUserPrompts: true, hiddenTools: defaultHiddenTools, toolBatchMaxLines: 12, maxTelegramChars: 3900 }
...
export function loadConfig(configPath = process.env.OPENCODEBOT_CONFIG || defaultConfigPath) {
  ...
  if (!fs.existsSync(requestedPath)) throw new Error(`Config file not found: ${requestedPath}`)
