...
State persistence avoids write amplification: duplicate mirror-marker updates are no-ops, and historical assistant markers skipped during catch-up are committed in one transaction per binding instead of one full `state.json` rewrite per message. Reconcile likewise avoids repeated full-history reads by paging backward to its in-memory high-water cursor or window boundary, rebuilding that cursor with one bounded catch-up after restart, gating unchanged sessions by OpenCodez `time.updated`, and retaining a bounded watchdog for missed SSE/update signals. These are fixed internal behavior-preserving optimizations rather than runtime tuning knobs.
