...
Every bound-session idle starts a short grace check; a user `message.updated` event also records the start early but is not required. If status and current message history still show no terminal answer, the bot sends one interrupted-run warning with an OpenCodez session button and uses that warning as the terminal queue signal. Recent periodic reconcile repeats the check to recover missed idle events or restarts. Once an outcome has been classified, the bounded handling ledger keeps its warning or expected-stop suppression idempotent; active trackers and queued prompt text remain memory-only. This behavior has no runtime config surface.
