...
`telegram.contextPairsByUser` is runtime-managed state for `/set_context`; it maps an allowed Telegram user id to a numeric default from 1 to 10. `/context` assembles prompt/final-answer text on demand from OpenCodez and never stores that text in `state.json`. Its Rich Message chunk size, 240,000-character total ceiling, and default of three pairs are fixed conservative behavior rather than runtime config knobs.
