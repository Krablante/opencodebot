...
`/notify_on`, `/notify_off`, and `/notify_status` control private DM notifications for `finalNotifications.userIds`. When enabled, the bot sends a short private message to each configured recipient when a final mirrored answer is ready. The DM names the topic, includes an `Open topic` button for the final mirrored message, and quotes the original user prompt in an expandable block for orientation. It does not include the final answer text.
