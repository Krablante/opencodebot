...
Telegram topic titles include a managed ` (<serverID>)` suffix only when two or more OpenCodez servers are configured. The suffix is applied to `/new`, web-created topics, backend title synchronization, manual topic renames, and `/reset`; changing server replaces the suffix without changing the stored base title. Single-server installations keep plain titles. The formatter reserves suffix space inside Telegram's 128-character title limit. Existing topics migrate lazily on their next title synchronization or reset instead of being renamed in one noisy bulk pass.
