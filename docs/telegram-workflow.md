...
`/debug_on`, `/debug_off`, and `/debug_status` control one global persistent final-DM diagnostics mode for the bot. The commands work from any allowed command context; when enabled, every final DM ends with an expandable `🐛 Run diagnostics` block showing agent-step p50/p95/max, exact completed/error ToolPart timing totals, failure count, and the three slowest raw tool names. Overall duration is intentionally not repeated because it already appears in the notification header. OpenCodez does not expose protocol origin in ToolPart metadata, so the block deliberately labels the combined metric `Tools/MCP` instead of guessing which non-built-in names came from MCP. `Σ` is cumulative tool time and may exceed overall duration when calls overlap. Debug diagnostics are omitted entirely from all final DMs when the global mode is off.
