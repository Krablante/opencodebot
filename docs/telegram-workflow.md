...
The bot syncs this slash-command menu on startup through Bot API `setMyCommands` for default, private-chat, group-chat, and configured-chat scopes, so the same commands should appear in Telegram's command suggestions.
