...
Incremental-reconcile checks cover `limit`/`before` cursor propagation, stopping at the previous complete-scan high-water message, and batching/no-op behavior for mirror-marker persistence. Keep these contracts in the existing focused tests and central smoke rather than introducing a separate performance-test framework.
