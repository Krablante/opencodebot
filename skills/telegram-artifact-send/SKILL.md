...
1. Identify exactly what should be delivered: an existing file, a newly created artifact, or a text snippet.
2. Keep file paths host-local. The plugin reads `path` on the host where the agent is running, then uploads bytes to the central opencodebot gateway.
...
6. Call `opencodebot_send_artifact` with `path` or `text`, `caption`, and only the needed optional fields.
